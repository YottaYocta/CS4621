...
static func march_cubes(voxels: PackedInt32Array, size: int, voxel_size: float) -> Dictionary:
    # given voxel pool array with coords voxels[x + size * y + size * size * z], this function will return a dictionary that maps indicies to triangles. It will use marching cubes to generate a valid, deterministic mesh.
    ...
    for z in range(size - 1):
        for y in range(size - 1):
            for x in range(size - 1):
                ...
                var cube_values = []
                cube_values.append(_get_voxel(voxels, x, y, z, size))
                cube_values.append(_get_voxel(voxels, x + 1, y, z, size))
                cube_values.append(_get_voxel(voxels, x + 1, y, z + 1, size))
                cube_values.append(_get_voxel(voxels, x, y, z + 1, size))
                cube_values.append(_get_voxel(voxels, x, y + 1, z, size))
                cube_values.append(_get_voxel(voxels, x + 1, y + 1, z, size))
                cube_values.append(_get_voxel(voxels, x + 1, y + 1, z + 1, size))
                cube_values.append(_get_voxel(voxels, x, y + 1, z + 1, size))
                ...
                if edge_flags & 1:
                    edge_vertices[0] = _interpolate_vertex(x, y, z, x + 1, y, z, cube_values[0], cube_values[1], voxel_size)
                if edge_flags & 2:
                    edge_vertices[1] = _interpolate_vertex(x + 1, y, z, x + 1, y, z + 1, cube_values[1], cube_values[2], voxel_size)
                if edge_flags & 4:
                    edge_vertices[2] = _interpolate_vertex(x + 1, y, z + 1, x, y, z + 1, cube_values[2], cube_values[3], voxel_size)
                if edge_flags & 8:
                    edge_vertices[3] = _interpolate_vertex(x, y, z, x, y, z + 1, cube_values[0], cube_values[3], voxel_size)
                if edge_flags & 16:
                    edge_vertices[4] = _interpolate_vertex(x, y + 1, z, x + 1, y + 1, z, cube_values[4], cube_values[5], voxel_size)
                if edge_flags & 32:
                    edge_vertices[5] = _interpolate_vertex(x + 1, y + 1, z, x + 1, y + 1, z + 1, cube_values[5], cube_values[6], voxel_size)
                if edge_flags & 64:
                    edge_vertices[6] = _interpolate_vertex(x + 1, y + 1, z + 1, x, y + 1, z + 1, cube_values[6], cube_values[7], voxel_size)
                if edge_flags & 128:
                    edge_vertices[7] = _interpolate_vertex(x, y + 1, z, x, y + 1, z + 1, cube_values[4], cube_values[7], voxel_size)
                if edge_flags & 256:
                    edge_vertices[8] = _interpolate_vertex(x, y, z, x, y + 1, z, cube_values[0], cube_values[4], voxel_size)
                if edge_flags & 512:
                    edge_vertices[9] = _interpolate_vertex(x + 1, y, z, x + 1, y + 1, z, cube_values[1], cube_values[5], voxel_size)
                if edge_flags & 1024:
                    edge_vertices[10] = _interpolate_vertex(x + 1, y, z + 1, x + 1, y + 1, z + 1, cube_values[2], cube_values[6], voxel_size)
                if edge_flags & 2048:
                    edge_vertices[11] = _interpolate_vertex(x, y, z + 1, x, y + 1, z + 1, cube_values[3], cube_values[7], voxel_size)

                # Build triangles from the lookup table
                var tri_list = TRI_TABLE[cube_index]
                for i in range(0, tri_list.size(), 3):
                    if i + 2 < tri_list.size():
                        var v1 = edge_vertices[tri_list[i]]
                        var v2 = edge_vertices[tri_list[i + 1]]
                        var v3 = edge_vertices[tri_list[i + 2]]
                        ...
                        triangles.append(idx + 1)
                        triangles.append(idx + 2)

    # Return the mesh data as a dictionary
    return {
        "vertices": PackedVector3Array(vertices),
        "triangles": PackedInt32Array(triangles)
    }

static func _get_voxel(voxels: PackedInt32Array, x: int, y: int, z: int, size: int) -> int:
    var index = x + size * y + size * size * z
    if index >= 0 and index < voxels.size():
        return voxels[index]
    return 0

static func _interpolate_vertex(x1: int, y1: int, z1: int, x2: int, y2: int, z2: int, val1: int, val2: int, voxel_size: float) -> Vector3:
